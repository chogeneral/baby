...
export function ContentTopicBoard({ topic, children }: Props) {
  ...
  /** 지정된 관리자 계정으로 로그인했을 때만 글쓰기 CTA 를 보여 준다 */
  const [showWriteLink, setShowWriteLink] = useState(false);
  const info = contentTopicPageInfo[topic];

  useEffect(() => {
    const session = readLoginSession();
    setShowWriteLink(isContentTopicWriteAllowedEmail(session?.email));
  }, [topic]);
  ...
  /* 정보 게시판은 기존 열 구성을 유지하고, 발달·부모이야기만 ‘글쓴이’ 열·모바일 닉네임을 보여 준다. */
  const showAuthorColumn = topic === "development" || topic === "parentStories";
